...
def process_identity_info(pid: int) -> dict:
    """
    Образ/путь/командная строка процесса БЕЗ привязки к какому-либо порту —
    та же информация, что и ``describe_process``, но для потребителей, у
    которых порта нет (GAP-311 Н1: ``standkit_hub.instance`` подтверждает
    личность pid перед жёстким убийством при перехвате, вне контекста
    "стенд на порту"). Тонкая обёртка над ``_windows_process_info``/
    ``_posix_process_info`` — они и так не используют порт для сбора улик.
    """
    return _windows_process_info(pid) if sys.platform == "win32" else _posix_process_info(pid)
