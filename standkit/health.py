...
def check_stand(
    ...
    *,
    ...
    if stand.host_kind in (HostKind.IIS, HostKind.DOCKER):
        # Проба «процесс» для iis/docker консультируется с бэкендом хостинга
        # (состояние App Pool / контейнера), а не с pidfile standkit — у этих
        # видов хостинга своего pidfile нет (см. ADR-0001). Сохраняем fallback
        # на TCP-порт: backend.is_running() уже пробует его сам, но если он
        # бросит исключение (появится в будущем реализация без внутреннего
        # фолбэка) — подстраховываемся ещё раз здесь.
