...
KIND_TITLES = {
    ...
    # ADR-0048/GAP-279: сайдкар подписи установщика несёт "kind", НЕ совпадающий с
    # тем, что ожидает ЭТОТ поток (например, поток установщика получил сайдкар с
    # kind="server", или наоборот). Тот же принцип, что у artifact_type_mismatch
    # выше (подпись доказывает подлинность, не роль), но отдельный kind: причина
    # ИНАЯ (перепутанный/подменённый канал публикации, не сборка не того типа
    # файла) и заслуживает своего текста в UI/логе. Не retriable — следующий тик
    # найдёт тот же сайдкар, чинится только переизданием на стороне издателя.
    "artifact_kind_mismatch": ("Артефакт подписан для другой роли — не тот, что "
                               "ожидался этим потоком", False, True),
    # ADR-0048/GAP-279: канал установщика не сконфигурирован на бэкенде ЛИБО для
    ...
    "installer_not_available": ("Установщик для этой версии не опубликован издателем",
                                True, False),
    # GAP-279: хаб без прав администратора не может запустить установщик, которому
    # они нужны (установка «для всех пользователей», WinError 740). Не retriable:
    # повтор из того же процесса упрётся в то же самое; чинит человек — перезапуском
    # диспетчера с правами или ручным запуском установщика.
    "elevation_required": ("Установщику нужны права администратора", False, True),
    # GAP-523/GAP-288: узкие каналы диспетчера (kind=hub) и скиллов/плагина
    # (kind=skills) — симметрично installer_not_available: издатель ещё не
    # выложил файл для этой версии/канал не сконфигурирован. Штатный пропуск,
    # не ошибка.
    "hub_not_available": ("Обновление диспетчера не опубликовано издателем", True, False),
    "skills_not_available": ("Обновление скиллов/плагина не опубликовано издателем",
                             True, False),
    # Самообновление exe доступно ТОЛЬКО замороженной (PyInstaller) сборке
    # диспетчера — pip-установку канал не подменяет (контракт GAP-523):
    # команда pip показывается человеку, запускать её сам канал не имеет
    # права. Не retriable — режим установки не меняется без переустановки.
    "self_update_unsupported": ("Самообновление доступно только для exe-сборки диспетчера",
                                False, True),
    # GAP-288: применение скиллов к найденному клиенту (CLI `setup
    # skills-install`) вернуло ненулевой код — маркер не пишется, отчёт
    # уходит пользователю целиком. Retriable: повтор той же командой может
    # починить (например, если харнесс был открыт и блокировал файл).
    "skills_apply_failed": ("Не удалось применить скиллы к найденным клиентам", True, True),
    # --- транспорт ------------------------------------------------------------------
