...
KIND_TITLES = {
    ...
    # GAP-463: издатель объявил версию поставленной ТОЛЬКО через установщик
    # (`requires_installer` в `GET /v1/version/latest`). Канал доставляет РОВНО ОДИН
    # файл — бинарь сервера; релиз, меняющий блок запуска, требования рантайма или
    # состав поставки вне бинаря, тихой подменой файла доставлен быть не может. Не
    # retriable: следующий тик найдёт ТОТ ЖЕ флаг у той же версии, повтор ничего не
    # изменит — установщик может запустить только человек.
    "requires_installer": ("Обновление ставится установщиком — тихая доставка "
                          "невозможна", False, True),
    # --- транспорт ------------------------------------------------------------------
