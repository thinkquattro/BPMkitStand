...
# ======================================================================================
# Маркер работающего процесса MCP (GAP-444)
# ======================================================================================
...
def read_runtime_marker() -> Optional[dict]:
    ...
    ещё ни разу не перезапускался после апгрейда с GAP-444), битый JSON, нет прав на
    ...
    Формат — контракт серверной ветки (GAP-444): `{"version": str, "started_at":
...
def _reconcile_restart_required(state) -> None:
    ...
    (GAP-444). Раньше `restart_required` выставлялся в `True` подменой файла
...
def check(client, state, ctx) -> dict:
    ...
    # GAP-444/GAP-442: локальная сверка маркера и попутный запрос нотсов — ДО сетевого
