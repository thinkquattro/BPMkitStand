"""
Федеративный клиент GUI: агрегирует стенды из локального ядра (transport=local)
и с удалённых агентов (transport=agent, по HTTP) в единый список для отрисовки.

Намеренно НЕ импортирует PySide6 — это чистый сетевой/доменный слой, который
можно тестировать и переиспользовать (например, из CLI) без Qt. Используется
только stdlib (``urllib``) — как и standkit_agent.server, без сторонних HTTP-клиентов.
...
TODO(следующая итерация):
  ...
  - кэширование/дебаунс частых опросов (polling из GUI по таймеру);
...
class FederatedClient:
    """
    Единая точка входа для GUI: даёт список стендов реестра со статусами,
