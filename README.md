...
- **Усыновление стендов** — стенд, поднятый вне диспетчера (руками из консоли, скриптом), можно остановить и перезапустить: диспетчер проверяет принадлежность процесса стенду и берёт его под управление только с явного подтверждения.
- **Виды хостинга** — kestrel, IIS, Docker, Kubernetes: диспетчеризация по полю `host_kind`, прозрачно для дашборда.
- **Тёмная и светлая тема** с переключателем.
- **Установка как приложение (PWA)** — отдельное окно без адресной строки, своя иконка; плюс компактный режим-виджет `?view=compact`.
...
Виды хостинга (kestrel / IIS / Docker / Kubernetes) — см. [docs/HOSTING.md](docs/HOSTING.md).
...
Или напрямую из репозитория (свежий `main`):
...
Откроется браузер с локальным дашбордом. Кнопка **«?»** в шапке открывает
**кукбук** — интерактивную инструкцию в одном самодостаточном HTML-файле
(установка, работа с дашбордом, FAQ по IIS и усыновлению стендов, полный
раздел про удалённые агенты). Файл лежит в репозитории —
[`standkit_hub/web/cookbook.html`](standkit_hub/web/cookbook.html), работает
офлайн и открывается двойным кликом с диска.

Реестр стендов берётся из
...
- [`standkit_hub/web/cookbook.html`](standkit_hub/web/cookbook.html) — **кукбук**: интерактивная инструкция одним файлом для аналитика и разработчика (кнопка «?» в шапке дашборда).
...
> Статус: `standkit` — молодой проект (0.6.x). Ядро/агент/хаб работоспособны, но
