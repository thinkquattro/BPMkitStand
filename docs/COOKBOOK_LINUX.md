...
Здесь ждёт ловушка. Для стенда есть поле `verify_tls`, а для **агента** такого
поля нет: клиент дашборда идёт к `agent_url` со штатной проверкой цепочки.
С self-signed сертификатом агента дашборд получит:
...
Стенд при этом жив, агент отвечает, токен верный — ломается только доверие к
сертификату. Два рабочих решения:

1. Указать сертификат агента (или его CA) переменной окружения **до** запуска
   дашборда:

   ```powershell
   $env:SSL_CERT_FILE = "C:\standkit\agent.crt"
   standkit-hub
   ```

   Переменная читается при инициализации `ssl` — выставлять её в уже запущенном
   процессе бесполезно.

2. Выпустить сертификат агента доверенным в вашем контуре CA и импортировать
   этот CA на машины операторов — правильный путь для постоянной эксплуатации.

После этого статус приходит нормально:

```console
{"name": "stand-a", "process": "ok", "http": "ok", "db": "unknown",
 "redis": "unknown", "last_deploy": "unknown", "details": {}}
```
...
агентом 0.7.0 на Linux — контракт HTTP обратно совместим. Помните только, что
новые поля реестра (`stand_scheme`, `verify_tls`) обрабатывает **та сторона,
которая делает пробу**, то есть агент: обновлять в первую очередь нужно его.
...
| `RemoteCallError: ... CERTIFICATE_VERIFY_FAILED ... self-signed certificate` | дашборд не доверяет сертификату агента | `SSL_CERT_FILE` с сертификатом/CA агента до запуска хаба, либо CA в доверенных |
