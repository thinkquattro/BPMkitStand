...
- **IIS**: платформа — только Windows. ⚠️ **Диспетчер (`standkit-hub`) должен
  ...
  **недостаточно**. Как запускать elevated:

  - **из самого дашборда** — щит «без прав администратора» в шапке (виден, только
    когда прав действительно нет) или кнопка «Перезапустить с правами администратора»
    в сообщении об ошибке: подтвердите окно Windows, и диспетчер поднимется заново на
    том же адресе, вкладка переподключится сама. Запущенные стенды перезапуск не
    трогает. Если окно подтвердили под другой учётной записью — диспетчер откажет с
    пояснением и продолжит работать без прав;
  - **только одну операцию** — «Только эту операцию» в сообщении об ошибке или блок
    «Однократная операция с правами администратора» в «Настройки → О программе»:
    старт/стоп/перезапуск IIS-стенда с запросом прав, сам диспетчер остаётся без них.
    Разбор обоих путей — [ADR-0006](adr/0006-hub-elevated-restart.md);
  - ярлык/консоль → правый клик → «Запуск от имени администратора» (elevated
    экземпляр заменит собой уже работающий обычный; до 0.11.0 он молча открывал
    браузер на старом, и казалось, что запуск не сработал);
  - раз и навсегда — свойства ярлыка → «Дополнительно» → галка «Запуск от
    имени администратора»; либо `standkit-hub` из уже поднятой
    административной консоли.

  Диспетчер в такой ситуации отдаёт понятную подсказку «запустите от имени
  администратора» в тексте ошибки. Живая приёмка — на стенде с реальным IIS.
