...
- **IIS**: платформа — только Windows. ⚠️ **Диспетчер (`standkit-hub`) должен
  ...
  **недостаточно**. Как запускать elevated:
  ...
  - **из самого дашборда** — кнопка «🛡 без прав администратора» в шапке
    (видна, только когда прав действительно нет): подтвердите запрос UAC, и
    диспетчер поднимется заново на том же адресе, а вкладка переподключится
    сама. Запущенные стенды перезапуск не трогает. Разбор —
    [ADR-0005](adr/0005-hub-elevated-restart.md);
  - ярлык/консоль → правый клик → «Запуск от имени администратора» (elevated
    экземпляр заменит собой уже работающий обычный — до версии с ADR-0005 он
    молча открывал браузер на старом, и казалось, что запуск не сработал);
